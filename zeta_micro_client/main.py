...
class ZetaMicroClient(object):
    ...
    def create_card_dispatch(self, 
        ...
        valid_data["customer"]["person_id"] = str(
            valid_data["customer"]["person_id"])
        valid_data["receiver"]["person_id"] = str(
            valid_data["receiver"]["person_id"])

        return self.zeta_service.create_card_dispatch(**valid_data)

    def find_card_dispatch(self, person_id: str, card_form_factor_id: str, secondary_person_id: str=None, realtime=False):
        return self.zeta_service.find_card_dispatch(person_id=person_id, card_form_factor_id=card_form_factor_id, secondary_person_id=secondary_person_id, realtime=realtime)
    
    def get_card_dispatch(self, card_dispatch_id: str):
        return self.zeta_service.get_card_dispatch(card_dispatch_id)
    
    def edit_card_dispatch_action(self, card_dispatch_id: str, action: str, attributes: dict):
        return self.zeta_service.edit_action_card_dispatch_action(card_dispatch_id, action, attributes)
